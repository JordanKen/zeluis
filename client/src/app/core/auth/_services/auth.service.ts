...
export class  AuthService {
  ...
  constructor(private handler: HttpBackend, private cookieService: CookieService) {
    this.http = new HttpClient(handler);
  }

  // Authentication/Authorization
  login(email: string, password: string): Observable<any> {
    return this.http.post<any>(API_USERS_URL + 'auth/users/login', {email, password});
  }
  getUserByToken(): Observable<any> {
    ...
    return this.http.get<any>(API_USERS_URL + 'auth/users/me', {headers: headers});
  }
  isAdmin(): Observable<any> {
    ...
    httpHeaders = httpHeaders.set('Authorization', 'Bearer ' + userToken);
    return this.http.get<any>(API_USERS_URL + 'auth/users/admin', {headers: httpHeaders});
  }
  ...
  getAllUsers(): Observable<any | User[]> {
    return this.http.get<any | User[]>(API_USERS_URL + 'v1/admin/users/getall');
  }
